...
preparation, and the role-facing CLI all consume it. `RoleConfigRow` independently carries an overlapping, larger
description during workspace preparation. Other closed squad-wide concepts, such as receive mode and member status,
are represented as strings at several boundaries.
...
Issue 024 is already changing the underlying model from role-centered to squad-member-centered. Performing this
assembly extraction at the same time would compete with that work, duplicate renames, and risk extracting transient
types before their responsibilities and names have settled.

## Goal

After issue 024 is complete, introduce a dependency-free `squad.Domain` assembly containing only the stable,
immutable vocabulary shared across the product. Move the resulting squad and member descriptors into that assembly,
replace any surviving `RoleRow`/`RoleConfigRow` duplication with one canonical type, and replace closed string values
...
member-local state ownership, addressing, configuration shape, and naming across the product. Complete that issue
...
Use the names established by issue 024. If the current `RoleRow` and `RoleConfigRow` still exist after that work,
replace them with the one canonical member descriptor rather than moving either `Row` type unchanged. Do not retain
aliases, wrappers, or duplicate compatibility records for internal callers.
...
`SquadConfiguration`, `SquadRoleConfiguration`, `SquadAgentConfiguration`, their JSON document types, configuration
exceptions, and loaders remain in `squad.Configuration`. They model and validate persisted input. Configuration and
workspace preparation map validated input and resolved paths into domain values; the domain assembly does not read
configuration files or resolve filesystem paths.
...
1. Complete issue 024 and inventory its resulting squad, role, member, receive-mode, and status representations.
2. Add `squad.Domain` to the solution with no project references.
3. Move the stable identities and immutable descriptors without changing their established semantics.
4. Consolidate any remaining parallel member records and update consumers to depend on the canonical domain value.
5. Replace authoritative receive-mode and member-status strings internally with closed domain enums while preserving
   existing configuration and protocol representations.
...
- Issue 024 is complete before implementation of this issue begins.
- `squad.Domain` exists in the solution and has no project references.
- The assembly contains only stable squad-wide identities, immutable descriptors, and closed domain vocabularies.
- The configured, resolved squad member has one canonical representation; no surviving `RoleRow`, `RoleConfigRow`,
  or equivalent parallel row records remain.
