...
**Status: changes requested (d748d5b0e6)**

#### Review findings on d748d5b0e6

**Finding 1 — high**

- **Location:** `src/squad.Specs/Support/HeadlessUiClient.cs` (`WaitForNoPendingPermissionAsync`),
  `src/squad.Specs/Features/AbortSequencingAndTerminalRoleFailure.feature`
  (A terminal session failure affects only its role and rejects later commands).
- **Violated behavior:** A terminal session failure must remove that role's pending interactions from published UI
  state. After the permission is observed pending and the coder session fails, the Then must prove current published
  state no longer contains `permission-1`.
- **Root cause:** `WaitForNoPendingPermissionAsync` uses `WaitForMessageAsync` to match any `state.snapshot` whose
  permissions array lacks that id. The `ui.ready` handshake snapshot already lacks `permission-1`, so the wait
  returns immediately without observing a snapshot after the failure. A session failure that leaves the permission
  published still passes.
- **Required outcome:** After the permission has been observed pending, wait until the latest published
  `state.snapshot` no longer contains it. Do not treat an earlier snapshot from before the request existed as proof
  of removal.
