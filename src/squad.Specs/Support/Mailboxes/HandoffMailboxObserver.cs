...
public sealed class HandoffMailboxObserver
{
    ...
    internal string SingleNewInboxRawContent(string recipientRole)
    {
        ...
        return File.ReadAllText(Directory.GetFiles(newDir, "*" + FileSuffix, SearchOption.TopDirectoryOnly).Single());
    }
        ...
        return new[] { "new", "in_process" }
            .SelectMany(state =>
            {
                ...
                return Directory.Exists(directory)
                    ? Directory.GetFiles(directory, "*" + FileSuffix, SearchOption.TopDirectoryOnly)
                    : [];
            })
    ...
    {
        var outbox = Path.Combine(myWorkspace.RoleWorktreePath(senderRole), ".blaxquad", "handoffs", "outbox");
        return Path.GetFileName(Directory.GetFiles(outbox, "*" + FileSuffix, SearchOption.TopDirectoryOnly).Single());
    }
