...
/// </summary>
public sealed class BackendScenarioAgent(FakeProviderControlServer control, string role)
{
    /// <summary>Waits until this role's session has reported a prompt across the control pipe and returns its
    /// content.</summary>
    public Task<string> WaitForPromptAsync(TimeSpan? timeout = null) => control.WaitForPromptAsync(role, timeout);

    /// <summary>Sends a semantic assistant reply for this role's session, awaiting the real production transcript
    /// projection to happen inside the launched process before returning.</summary>
    public Task ReplyAsync(string content, TimeSpan? timeout = null) => control.ReplyAsync(role, content, timeout);
}
