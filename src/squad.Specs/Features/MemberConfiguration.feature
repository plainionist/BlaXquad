Feature: Configure and launch reusable roles with distinct members
  ...
  referencing an undeclared role, or a legacy version-1 document - with a specific diagnostic before any member
  session starts.
  ...
  Scenario: Two members sharing one role start independently and both read the same role prompt
    ...
    Then the backend scenario observes a session started for role "coder-a" across the control pipe
    And the backend scenario observes a session started for role "coder-b" across the control pipe
  ...
  Scenario: A legacy version-1 configuration is rejected with an explicit migration diagnostic
