Feature: UI protocol validation
  ...
  rejected before it ever reaches the role's provider session, and never disturbs the process's ability to keep
  ...
  Scenario Outline: An invalid UI message is rejected before reaching the role's session
    ...
    Examples:
      ...
      | unknown type                      | {"type":"unknown"}                                                                                      | Unknown UI message type 'unknown'. |
      | missing role                      | {"type":"prompt.send","payload":{"prompt":"hello"}}                                                     | The UI message is missing role. |
      | missing request ID                | {"type":"permission.respond","role":"coder","payload":{"approved":true}}                                | The UI message is missing requestId. |
      | invalid string payload            | {"type":"prompt.send","role":"coder","payload":{"prompt":42}}                                           | The UI message is missing payload.prompt. |
      | invalid boolean payload           | {"type":"permission.respond","role":"coder","requestId":"permission-1","payload":{"approved":"yes"}}     | The UI message is missing payload.approved. |
      | invalid integer payload           | {"type":"transcript.page","role":"coder","payload":{"beforeIndex":"five"}}                              | The requested operation requires an element of type 'Number', but the target element has type 'String'. |
      | invalid synchronization payload   | {"type":"transcript.synchronize","payload":{"roles":"coder"}}                                           | The UI message contains invalid transcript positions. |
