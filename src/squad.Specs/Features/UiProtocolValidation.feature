Feature: UI protocol validation
  ...
  Scenario Outline: An invalid UI message is rejected before reaching the role's session
    ...
    Examples:
      ...
      | missing type                      | {"version":6}                                                                                            | The UI message is missing a type. |
      | unknown type                      | {"version":6,"type":"unknown"}                                                                           | Unknown UI message type 'unknown'. |
      | missing role                      | {"version":6,"type":"prompt.send","payload":{"prompt":"hello"}}                                         | The UI message is missing role. |
      | missing request ID                | {"version":6,"type":"permission.respond","role":"coder","payload":{"approved":true}}                    | The UI message is missing requestId. |
      | invalid string payload            | {"version":6,"type":"prompt.send","role":"coder","payload":{"prompt":42}}                               | The UI message is missing payload.prompt. |
      | invalid boolean payload           | {"version":6,"type":"permission.respond","role":"coder","requestId":"permission-1","payload":{"approved":"yes"}} | The UI message is missing payload.approved. |
      | invalid integer payload           | {"version":6,"type":"transcript.page","role":"coder","payload":{"beforeIndex":"five"}}                  | The requested operation requires an element of type 'Number', but the target element has type 'String'. |
      | invalid synchronization payload   | {"version":6,"type":"transcript.synchronize","payload":{"roles":"coder"}}                                | The UI message contains invalid transcript positions. |
